...
export default function Navbar() {
  ...
  return (
    <>
      <nav className={styles["navbar"]}>
        ...
        {mobile && (
          <div className={styles["hamburger"]}>
            {navVisibility ? (
              // <IoCloseSharp />
              <IoMdClose onClick={toggleNavVisibility} size="30" />
            ) : (
              <SlMenu onClick={toggleNavVisibility} size="30" />
            )}
          </div>
        )}
      </nav>
      {mobile && (
        <nav
          className={`${styles["nav-menu-mobile"]} ${
            navVisibility ? "" : styles["nav-menu-mobile-hidden"]
          }`}
        >
          <Link
            className={
              pathname === "/"
                ? styles["nav-link-mobile-active"]
                : styles["nav-link-mobile"]
            }
            href="/"
